...
app.get("/posts/:id",(req,res)=>{
  ...
  const findIndex=posts.find((p)=> p.id=== id)
  if(!findIndex) return res.status(404).json({message:`cannot find post`})
    res.json(posts)
})
...
app.patch("/posts/:id", (req, res) => {
  ...
  res.json({ message: "Post updated", post: findPost });
});
...
app.delete("/posts/:id",(req,res)=>{
  const id = parseInt(req.params.id)
  const theId = posts.findIndex((p)=> p.id === id)

  if(theId === -1) return res.status(404).json({message:"post not found"})
    posts.splice(theId,1)
    console.log(`removed posts with id: ${theId}`)
